...
The installation of Pipeless has been split into different flavors.

Now, there is a default flavor `pipeless-ai` and two flavors that will install the ONNX dependencies, one for CPU (`pipeless-ai[onnx-cpu]`) and one for GPU (`pipeless-ai[onnx-gpu]`).

### Core version `0.1.10` and CLI version `0.1.8`

The plugin system was rebuilt. These versions include the new plugin system. If you were not using plugins, no changes are required.
