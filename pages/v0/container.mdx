...
* `pipeless-ai/pipeless:{version}`: Default image with enough dependencies to play and test Pipeless on CPU.
* `pipeless-ai/pipeless:{version}-cuda`: Image to run Pipeless with CUDA support for running inference. When using this image you need to install the Nvidia Container Toolkit on your system.
* `pipeless-ai/pipeless:{version}-tensorrt`: Image to run Pipeless using TensorRT based GPUs. When using this image you need to install the Nvidia Container Toolkit on your system.
...
```bash copy
docker run --rm pipeless-ai/pipeless --help
```

Create a new project locally:

```bash copy
docker run --rm -v /your/app/parent/dir:/app pipeless-ai/pipeless create project my_app_name
```
<Callout>
...
```bash copy
docker run --rm -v /your/app/dir:/app pipeless-ai/pipeless run all
```

Run input only:

```bash copy
docker run --rm pipeless-ai/pipeless run input
```

### GPU usage with CUDA

Use the image `pipeless-ai/pipeless:latest-cuda`. To properly use CUDA inside the container, your host system needs to have `nvidia-container-toolkit` installed. Run the following commands to install it:
...
```bash copy
docker run --gpus all -v /your/app/dir:/app  pipeless-ai/pipeless:latest-cuda run 
```
...
```bash copy
docker run --gpus all --entrypoint nvidia-smi pipeless-ai/pipeless:latest-cuda
```
...
```bash copy
docker run --rm -e "PIPELESS_USER_PYTHON_PACKAGES=opencv-python;some_other_package" pipeless-ai/pipeless run worker
```
