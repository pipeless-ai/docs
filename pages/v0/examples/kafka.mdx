...
To connect to Kafka we will use the **Pipeless Kafka Plugin**. This plugin is a wrapper around the `confluent_kafka` package for a more convenient usage, but you could modify the example to use any Kafka client directly from your the hooks implementation or even replace Kafka by any event streaming platform or messaging broker. Check the whole documentation of the [Pipeless Kafka Plugin](/docs/v0/plugins/kafka).
...
<Callout>
    If you don't have the required dependencie for Pipeless installed check the [installation guide](/docs/v0/install) or [use Docker](/docs/v0/install#using-docker-) instead.
</Callout>
...
Simply export the following env var. If you used the provided `docker-compose.yaml`, just copy paste the command below, if not, you need to edit the address:
...
This is a step by step description of what every line of the application code is doing. This time, it describes only the new lines related to the Kafka example, to understand how the recognition model is used please check the [cats example walkthrough](/docs/v0/examples/cats/#walkthrough).
...
<Callout>The bounding boxes detection is exactly the same than on the cats example.</Callout>
