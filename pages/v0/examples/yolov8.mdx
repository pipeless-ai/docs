...
<Callout>
This example uses the YOLOv8 SDK. The Pipeless worker is now shipping a [built-in ONNX Runtime](/docs/v0/inference) and we recommend to stick to it, which will help you creating lighter applications and run them on GPUs out fo the box. That said, nothing prevents you from loading frameworks on your Pipeless application.
</Callout>
...
<Callout>
    If you don't have the required dependencies for Pipeless installed check the [installation guide](/docs/v0/install) or [use Docker](/docs/v0/install#using-docker-) instead.
</Callout>
