...
Pipeless also has a plugins registry to make it easy for you to manage plugins in your project. You can list all the available plugins in registry by running:
...
The `plugin_id` is the ID of the plugin in the plugins registry, which is easy to obtain with the `list` command.
