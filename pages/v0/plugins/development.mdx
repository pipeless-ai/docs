...
When creating a plugin, you must be aware of an important difference between the how the plugin `__init__` method and `before` hook are executed.
The **`__init__`** method is invoked **before marking the worker as ready**. This allows you to perform actions that take some time without lossing frames, such as downloading files, connecting to external services, etc.
The **`before`** hook is called **before the first frame of each stream**. Within the `before` hook you can implement, for example, logic to reset the plugin state before each new stream.
