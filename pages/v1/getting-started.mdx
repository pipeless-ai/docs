...
* `stateless` (default): A stateless hook can run multiple instances of the hook at the same time for different frames.
...
* `stateful`: Stateful hooks are the kind of hook you want to use when some internal state is required. A good example of when to use a stateful hook is
    ...
    hooks, since the internal state is locked and released on each frame to avoid corrupting it.
