...
* `stateless` (default): Stateless hooks can run at the same time for different frames.
...
* `stateful`: Stateful hooks are the kind of hook you want to use when some internal state is required. A good example of when to use a stateful hook is
    when you use an object tracking model, since these models maintain internal state. Stateful hooks introduce extra latency compared to stateless
    hooks, since the internal state is locked and released on each frame to avoid corrupting the internal state.
